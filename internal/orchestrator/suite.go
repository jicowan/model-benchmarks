...
func (o *Orchestrator) ExecuteSuite(ctx context.Context, suiteRunID string, req database.SuiteRunRequest) {
	...
	cfg := RunConfig{
		...
		Request: &database.RunRequest{
			...
			HfToken:                req.HfToken,
			// PRD-50: forward streamer knobs so the single suite deployment
			// honors them the same way a one-off run does.
			StreamerMode:           req.StreamerMode,
			StreamerConcurrency:    req.StreamerConcurrency,
			StreamerMemoryLimitGiB: req.StreamerMemoryLimitGiB,
		},
	}

...
var crdGVRTable = map[string]schema.GroupVersionResource{
	...
	// InferencePool (+ Endpoint Picker) is deferred to PRD-58 — PRD-56 routes
	// the gateway straight to the leader Service.
...
// can delete exactly what was created (PRD-56 Layer 2).
...
func (o *Orchestrator) applyUnstructured(ctx context.Context, ns, docJSON string) (appliedObject, error) {
	...
	return appliedObject{gvr: gvr, name: name}, nil
}
...
func (o *Orchestrator) deleteUnstructured(ctx context.Context, ns string, obj appliedObject) error {
	...
	err := o.dynClient.Resource(obj.gvr).Namespace(ns).Delete(ctx, obj.name, metav1.DeleteOptions{

...
func (r *Repository) UpdateScenarioResult(ctx context.Context, result *ScenarioResult) error {
	...
	case "completed":
		query = `UPDATE scenario_results SET
		    ...
		    ttft_p50_ms = $3, ttft_p90_ms = $4, ttft_p95_ms = $5, ttft_p99_ms = $6,
		    e2e_latency_p50_ms = $7, e2e_latency_p90_ms = $8, e2e_latency_p95_ms = $9, e2e_latency_p99_ms = $10,
		    itl_p50_ms = $11, itl_p90_ms = $12, itl_p95_ms = $13, itl_p99_ms = $14,
		    tpot_p50_ms = $15, tpot_p90_ms = $16, tpot_p99_ms = $17,
		    throughput_tps = $18, requests_per_second = $19,
		    successful_requests = $20, failed_requests = $21,
		    waiting_requests_max = $22,
		    accelerator_utilization_pct = $23, accelerator_utilization_avg_pct = $24,
		    accelerator_memory_peak_gib = $25, accelerator_memory_avg_gib = $26,
		    sm_active_avg_pct = $27, sm_active_peak_pct = $28,
		    tensor_active_avg_pct = $29, tensor_active_peak_pct = $30,
		    dram_active_avg_pct = $31, dram_active_peak_pct = $32,
		    loadgen_config = $33
		 WHERE id = $34`
		args = []any{
			result.Status, time.Now(),
			result.TTFTP50Ms, result.TTFTP90Ms, result.TTFTP95Ms, result.TTFTP99Ms,
			result.E2ELatencyP50Ms, result.E2ELatencyP90Ms, result.E2ELatencyP95Ms, result.E2ELatencyP99Ms,
			result.ITLP50Ms, result.ITLP90Ms, result.ITLP95Ms, result.ITLP99Ms,
			result.TPOTP50Ms, result.TPOTP90Ms, result.TPOTP99Ms,
			result.ThroughputTPS, result.RequestsPerSecond,
			result.SuccessfulRequests, result.FailedRequests,
			result.WaitingRequestsMax,
			result.AcceleratorUtilizationPct, result.AcceleratorUtilizationAvgPct,
			result.AcceleratorMemoryPeakGiB, result.AcceleratorMemoryAvgGiB,
			result.SMActiveAvgPct, result.SMActivePeakPct,
			result.TensorActiveAvgPct, result.TensorActivePeakPct,
			result.DRAMActiveAvgPct, result.DRAMActivePeakPct,
...
func (r *Repository) GetScenarioResults(ctx context.Context, suiteRunID string) ([]ScenarioResult, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(
			...
			&r.TTFTP50Ms, &r.TTFTP90Ms, &r.TTFTP95Ms, &r.TTFTP99Ms,
			&r.E2ELatencyP50Ms, &r.E2ELatencyP90Ms, &r.E2ELatencyP95Ms, &r.E2ELatencyP99Ms,
			&r.ITLP50Ms, &r.ITLP90Ms, &r.ITLP95Ms, &r.ITLP99Ms,
			&r.TPOTP50Ms, &r.TPOTP90Ms, &r.TPOTP99Ms,
			&r.ThroughputTPS, &r.RequestsPerSecond,
			&r.SuccessfulRequests, &r.FailedRequests,
			&r.WaitingRequestsMax,
			&r.AcceleratorUtilizationPct, &r.AcceleratorUtilizationAvgPct,
			&r.AcceleratorMemoryPeakGiB, &r.AcceleratorMemoryAvgGiB,
			&r.SMActiveAvgPct, &r.SMActivePeakPct,
			&r.TensorActiveAvgPct, &r.TensorActivePeakPct,
			&r.DRAMActiveAvgPct, &r.DRAMActivePeakPct,
